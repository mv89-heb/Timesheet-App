...
def compute_month_summary(month):
    ...
    for s in segments:
        if s['date'] < today_str and s['entry_time'] and not s['exit_time']:
            anomalies_count += 1
    ...
    for e in emps:
        employees_assigned_per_domain[e['department']] = employees_assigned_per_domain.get(e['department'], 0) + 1
...
def get_shifts(emp_id):
    ...
    # One DB connection for the entire read. The previous implementation called
    # get_setting() first, which opened a second Neon connection before querying
    # the actual shifts. On a sleeping Neon compute this added a large delay.
    with db_cursor(dict_cursor=True) as (conn, cursor):
        cursor.execute("SELECT value FROM settings WHERE key = 'shift_gap_alert_hours'")
        setting_row = cursor.fetchone()
        try:
            gap_alert_hours = float(setting_row['value']) if setting_row and setting_row.get('value') else 8.0
        except (TypeError, ValueError):
            gap_alert_hours = 8.0
        if gap_alert_hours <= 0:
            gap_alert_hours = None

        if month_filter:
            # Keep the month filter server-side and use the employee/date index.
            cursor.execute("""SELECT s.*, d.name AS domain_name, d.color AS domain_color
                              FROM shift_segments s
                              LEFT JOIN domains d ON d.id = s.domain_id
                              WHERE s.employee_id = %s AND s.date LIKE %s
                              ORDER BY s.date DESC, s.id DESC""", (emp_id, f"{month_filter}%"))
        else:
            cursor.execute("""SELECT s.*, d.name AS domain_name, d.color AS domain_color
                              FROM shift_segments s
                              LEFT JOIN domains d ON d.id = s.domain_id
                              WHERE s.employee_id = %s
                              ORDER BY s.date DESC, s.id DESC""", (emp_id,))
    ...
    result = []
    for date_str, segs in sorted(by_date.items(), reverse=True):
        segs_sorted = sorted(
            segs,
            key=lambda x: (t_to_float(x['entry_time']) if t_to_float(x['entry_time']) is not None else 999, x['id'])
        )
        warnings, total_hours, seg_list, prev_exit = [], 0.0, [], None
        for seg in segs_sorted:
            entry_f, exit_f = t_to_float(seg['entry_time']), t_to_float(seg['exit_time'])
            if date_str < today_str and seg['entry_time'] and not seg['exit_time']:
                warnings.append(f"משמרת פתוחה מ-{seg['entry_time']}")
            if seg['total_hours'] and float(seg['total_hours']) > 16:
                warnings.append(f"משמרת חריגה ({seg['total_hours']} שעות)")
            if prev_exit is not None and entry_f is not None:
                if entry_f < prev_exit:
                    warnings.append("חפיפת זמנים באותו יום")
                elif gap_alert_hours is not None:
                    gap = entry_f - prev_exit
                    if gap < 0:
                        gap += 24
                    if gap > gap_alert_hours:
                        warnings.append(f"פער של {round(gap, 1)} שעות")
            ...
            seg_list.append({
                ...
                'domain_name': seg['domain_name'] or 'ללא תחום',
                'domain_color': seg['domain_color'] or '#94a3b8',
                'entry': seg['entry_time'] or '',
                'exit': seg['exit_time'] or '',
                'total_hours': seg['total_hours'] or 0,
                'notes': seg['notes'] or '',
                'source': seg['source'] or 'manual'
            })
        result.append({
            'date': date_str,
            'segments': seg_list,
...
def upsert_shift():
    ...
    if not emp_id or not date or segments is None:
    ...
    with db_cursor() as (conn, cursor):
        ...
        for seg in segments:
            ...
            try:
                ...
                total = 0
            if not entry and not exit_ and not total and not seg.get('notes'):
                continue
            if not total and entry and exit_:
                total = calc_hours(entry, exit_)
            seg_id = seg.get('id')
...
def kiosk_validate_pin():
    ...
    if not emp.get('is_active', True):
        return jsonify({'success': False, 'message': 'העובד אינו פעיל במערכת.'})
...
def kiosk_punch():
    ...
    with db_cursor(dict_cursor=True) as (conn, cursor):
        ...
        if action_type == 'entry':
            if open_segment:
                return jsonify({'success': False, 'message': 'אתה כבר במשמרת פעילה.'})
            if not domain_id:
                return jsonify({'success': False, 'message': 'יש לבחור תחום.'})
            cursor.execute("SELECT id, name FROM domains WHERE id = %s AND active = TRUE", (domain_id,))
            domain = cursor.fetchone()
            if not domain:
                return jsonify({'success': False, 'message': 'תחום אינו תקין.'})
            cursor.execute("""INSERT INTO shift_segments (employee_id, date, domain_id, entry_time, exit_time, total_hours, notes, source)
                              VALUES (%s, %s, %s, %s, NULL, 0, NULL, 'kiosk')""", (emp['id'], today, domain_id, time_now))
            action_name, domain_label = "כניסה למשמרת", domain['name']
        elif action_type == 'exit':
            if not open_segment:
                return jsonify({'success': False, 'message': 'לא נמצאה משמרת פתוחה.'})
...
def kiosk_time_correction():
    ...
    with db_cursor(dict_cursor=True) as (conn, cursor):
        ...
        if not emp:
            return jsonify({'success': False, 'message': 'קוד PIN שגוי.'})
        if not emp.get('is_active', True):
            return jsonify({'success': False, 'message': 'העובד אינו פעיל.'})
        ...
        cursor.execute("""INSERT INTO time_corrections (employee_id, date, domain_id, entry_time, exit_time, reason)
                          VALUES (%s, %s, %s, %s, %s, %s)""", (emp['id'], date, domain_id, entry, exit_time, reason))
        conn.commit()
