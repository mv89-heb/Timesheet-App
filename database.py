...
# חיבור למסד הנתונים
# ==========================================
# הערה חשובה לגבי Neon: זהו מסד נתונים "serverless" שמשעה (suspend) את
# ה-compute שלו אחרי חוסר פעילות, וממילא כבר מריץ pooler (PgBouncer) משלו
# בנקודת הקצה "-pooler". מאגר חיבורים נוסף בצד שלנו (ThreadedConnectionPool)
# יצר pooling כפול, והחזיק חיבורים "פתוחים" מהצד שלנו אחרי ש-Neon כבר סגר
# אותם בפועל - זה גרם לשגיאות חיבור שנראו כמו "אין נתונים". לכן כל בקשה
# פותחת חיבור טרי משלה וסוגרת אותו בסיום - הדפוס הנכון מול ה-pooler הזה.
def get_db():
...
def migrate_legacy_shifts():
    ...
    with db_cursor(dict_cursor=True) as (conn, cursor):
        try:
            # בודקים אם יש בכלל היסטוריה לשחזר
            cursor.execute("SELECT COUNT(*) AS c FROM shifts")
            if cursor.fetchone()['c'] == 0:
                return

            # שליפת תחומי עבודה קיימים למיפוי
            ...
            # מיפוי היסטורי לפי הקובץ המקורי
            ...
            # שולפים פעם אחת אילו (עובד, תאריך) כבר קיימים ב-shift_segments, כדי לא
            # לבצע שאילתת בדיקה נפרדת לכל שורה (וגם לא לשכפל נתונים שכבר הועברו)
            ...
            for s in legacy_rows:
                ...
                for entry, exit_ in ((s['entry1'], s['exit1']), (s['entry2'], s['exit2'])):
                    ...
                    cursor.execute("""INSERT INTO shift_segments
                            ...
                            (s['employee_id'], s['date'], domain_id, entry, exit_val, round(hours, 2),
                             notes if first_segment else None))
                    first_segment = False
                    migrated += 1
...
def hebrew_calendar_day(year, month, day):
    if not HEBREW_CALENDAR_AVAILABLE: return None
    try: gd = pyluach_dates.GregorianDate(year, month, day)
